 ...
 */
public class TurtleSim extends AbstractNodeMain implements IRobot {
...
double x=0.0,y=0.0,theta=0.0;
  ...
  public void onStart(ConnectedNode connectedNode) {
    ...
    tf2.setRobot(this);
     tf2.fireUpScreen();
    Subscriber<geometry_msgs.Twist> subscriber = connectedNode.newSubscriber("/turtle1/cmd_vel", geometry_msgs.Twist._TYPE);
	...
	public void onNewMessage(geometry_msgs.Twist message) {
		...
		long dt=time2-time;
		x+=v.getX()*dt*Math.cos(theta);
		y+=v.getX()*dt*Math.sin(theta);
		
		x+=-v.getY()*dt*Math.sin(theta);
		y+=-v.getY()*dt*Math.cos(theta);
